"""
RETRO RAYCASTER
A Wolfenstein-3D-style pseudo-3D pixelated game built with pygame.ce.
Walls are covered in a procedurally generated stone-block texture
(no external image files needed), tinted per wall-color id. The
ceiling is a wrap-around night skybox with stars and a glowing moon
that pans as you turn but stays fixed relative to the world. The gun
itself is invisible (only a muzzle flash marks each shot); left-click
fires an instant hitscan shot down the crosshair for 20 damage, and
each enemy has a 100 HP bar that depletes until it dies. Enemies
slowly walk toward the player and their health bars are hidden
behind walls. Touching the player deals 15 damage; the player has
their own 100 HP bar in the top-right corner, and hitting 0 HP
brings up a Game Over screen with a clickable Respawn button. A new
enemy spawns on a random open tile every 15 seconds regardless of
how many are already alive; every 5 kills that interval drops by 5
seconds (15s -> 10s -> 5s), bottoming out at 5 seconds. At 20 kills,
regular spawning stops for good and a boss, The Revenant, appears
with 500 HP and a dedicated health bar at the top of the screen; it
hits for 25 damage on contact. Defeating it wins the game, bringing
up a victory screen with a Play Again button.

Controls
--------
  W / S       - move forward / backward
  A / D       - strafe left / right
  Left/Right  - turn (arrow keys)
  Mouse move  - turn (look around) -- mouse is captured, press TAB to release
...
TEX_SIZE = 64             # procedural texture resolution (square)
